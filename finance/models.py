...
class CashEntry(models.Model):
    ...
    ОПЛАТА ПОСТАВЩИКУ сюда пока НЕ попадает: приход материала система видит, но
    не знает, отдали за него деньги или взяли в долг (для долга есть свой вид
    расхода). Пока это так, остаток по кассе не включает закуп — его вносят
    записью руками.
